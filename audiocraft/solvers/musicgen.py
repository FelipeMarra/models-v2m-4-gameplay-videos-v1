...
class MusicGenSolver(base.StandardSolver):
    ...
    def build_model(self) -> None:
        ...
        #print("@@@@@ solvers.musicge.MusicGenSolver.build_model")
    ...
    def load_state_dict(self, state: dict) -> None:
        #print('########## solvers.musicge.MusicGenSolver.load_state_dict')
        # Looks like this is here just to shove a possible 'condition_provider' into the 'model' dict 
        if 'condition_provider' in state:
            #print("'condition_provider' in state")
        ...
        if 'compression_model' in state:
            #print("'compression_model' in state")
        ...
        #print('super().load_state_dict(state)')
    ...
    def evaluate_audio_generation(self) -> dict:
        ...
        # if self.cfg.evaluate.metrics.lp_text_consistency:
        #     lp_text_consistency = builders.get_lp_text_consistency(self.cfg.metrics.text_consistency).to(self.device)
        #     should_run_eval = True
        ...
        if should_run_eval:
            ...
            for idx, batch in enumerate(lp):
                ...
                # if lp_text_consistency is not None:
                #     videos_paths = [m.meta.video for m in meta]
                #     lp_text_consistency.update(y_pred, videos_paths, sizes, sample_rates)
