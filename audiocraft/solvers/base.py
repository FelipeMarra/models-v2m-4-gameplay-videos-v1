...
class StandardSolver(ABC, flashy.BaseSolver):
    ...
    def __init__(self, cfg: omegaconf.DictConfig):
        ...
        #print(f"\nself.cfg.optim.updates_per_epoch: {self.cfg.optim.updates_per_epoch}\n self.train_updates_per_epoch:{self.train_updates_per_epoch}\nself.total_updates:{self.total_updates}\n")
    ...
    def _get_state_source(self, name) -> flashy.state.StateDictSource:
        ...
        #print('solvers.base.StandardSolver._get_state_source')
    ...
    def register_best_state(self, *args: str):
        ...
        """
        #print('$$$$$ solvers.base.StandardSolver.register_best_state')
        for name in args:
            #print(f'name:{name}')
            state_source = self._get_state_source(name)
            #print(f'state_source[:10]:{list(state_source.state_dict().keys())[:10]}')
    ...
    def save_checkpoints(self):
        ...
        if self.cfg.checkpoint.save_every:
            #print(f"@@@@@ solvers.base.save_checkpoints save_every:")
            if epoch % self.cfg.checkpoint.save_every == 0:
                ...
                if self.cfg.checkpoint.keep_every_states is not None and len(self.cfg.checkpoint.keep_every_states) > 0:
                    #print(f"@@@@@ solvers.base.save_checkpoints save_every keep_every: {self.cfg.checkpoint.keep_every_states}:")

                    minimal_state = {
                        name: source for name, source in state.items()
                        if name in self.cfg.checkpoint.keep_every_states
                    }
                    print(f"minimal_state:\n{list(minimal_state['ema'].keys())}\n")
        ...
        if self.cfg.checkpoint.save_last:
            #print(f"@@@@@ solvers.base.save_checkpoints save_last:")
    ...
    def load_checkpoints(self, load_best: bool = False, ignore_state_keys: tp.List[str] = []) -> tp.Optional[dict]:
        ...
        """
        #print('@@@@@ solvers.base.StandartSolver.load_checkpoints')
        ...
        print(f"\n -----------------> continue_pretrained: {continue_pretrained} <----------------- \n")
        ...
        if rank0_checkpoint_path.exists():
            ...
            #print('self.cfg.continue_from and not continue_pretrained')
        ...
        if load_from_path is not None:
            #print('load_from_path is not None')
            state:dict = checkpoint.load_checkpoint(load_from_path, is_sharded)
            #print(f'sate: {state.keys()}')
            #print(f"sate model:\n{state['model']['condition_provider.conditioners.description.t5.shared.weight']}\n")
            #print(f"sate best_state:\n{state['best_state']['model']['condition_provider.conditioners.description.t5.shared.weight']}\n")
        ...
        if checkpoint_source is not None and checkpoint_source != checkpoint.CheckpointSource.CURRENT_XP:
            #print('checkpoint_source is not None and checkpoint_source != checkpoint.CheckpointSource.CURRENT_XP')
            ...
            # print(f'self._continue_best_source_keys:{self._continue_best_source_keys}') ['best_state', 'fsdp_best_state']
            #TODO(felipe)print(f"sate model:\n{state['best_state']['model']['condition_provider.conditioners.description.t5.shared.weight']}\n")
            ...
            if 'fsdp_best_state' in state and state['fsdp_best_state']:
                #print("'fsdp_best_state' in state and state['fsdp_best_state']")
    ...
    def restore(self, load_best: bool = False, replay_metrics: bool = False,
        ...
        #print('@@@@@ solves.base.StandartSolver.restore')
        #print(f'ignore_state_keys: {ignore_state_keys}') # EMPTY on running metrics
    ...
    def common_train_valid(self, dataset_split: str, **kwargs: tp.Any):
        ...
        # print(f"\n ======================= > common_train_valid updates_per_epoch: {updates_per_epoch}\n")
