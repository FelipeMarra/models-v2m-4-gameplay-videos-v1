...
def get_ema(module_dict: nn.ModuleDict, cfg: omegaconf.DictConfig) -> tp.Optional[optim.ModuleDictEMA]:
    ...
    """
    kw: tp.Dict[str, tp.Any] = dict(cfg) # type: ignore
...
def get_audio_datasets(cfg: omegaconf.DictConfig,
    ...
    """
    #print("@@@@@ solvers.builders.get_audio_datasets")
    dataloaders: dict = {}
    ...
    #print(f"sample_rate:{sample_rate}; channels:{channels}; seed:{seed}; max_sample_rate:{max_sample_rate}; max_channels:{max_channels}")
    ...
    # print(f"splits_cfg['train']:\n{splits_cfg['train']}\n")
    # print(f"splits_cfg['valid']:\n{splits_cfg['valid']}\n")
    # print(f"splits_cfg['evaluate']:\n{splits_cfg['evaluate']}\n")
    # print(f"splits_cfg['generate']:\n{splits_cfg['generate']}\n")
    ...
    for split, path in cfg.datasource.items():
        ...
        # print(f"\n###############################################################")
        # print(f"solver.builders.get_audio_datasets split:{split}; path:{path}")
        ...
        if kwargs.get('permutation_on_files') and cfg.optim.updates_per_epoch:
            #print(f"kwargs.get('permutation_on_files'):{kwargs.get('permutation_on_files')}")
            kwargs['num_samples'] = (
                flashy.distrib.world_size() * cfg.dataset.batch_size * cfg.optim.updates_per_epoch # type: ignore
            )
        ...
        #print(f"{split} num_samples:{num_samples}; shuffle:{shuffle}; batch_size:{batch_size}; num_workers:{num_workers}")

        if dataset_type == DatasetType.MUSIC:
            #print("DatasetType.MUSIC")
            # Will load AduioMeta (audio path, duration, etc.) and use it to instanciate a MusicDataset
        ...
        if split == 'train' and cfg.dataset.train.disable_sampling:
            num_samples = len(dataset)
        #print(f"{split} --------------------------> num_samples: {num_samples}; len(dataset): {len(dataset)}")
