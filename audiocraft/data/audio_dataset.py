...
class AudioMeta(BaseInfo):
    ...
    @classmethod
    def from_dict(cls, dictionary: dict):
        #print("@@@@@ data.audio_dataset.AudioMeta.from_dict")
...
def load_audio_meta(
    ...
    """
    #print(f"@@@@@ data.audio_data.load_audio_meta")
...
class AudioDataset:
    ...
    def __init__(
        ...
        #print("@@@@@ data.audio_dataset.AudioDataset.__init__")
        ...
        self.sampling_probabilities = self._get_sampling_probabilities() # Uniform distribution for the settings I have now
    ...
    def sample_file(self, index: int, rng: torch.Generator) -> AudioMeta:
        ...
        """
        #print(f"$$$$$ data,audio_dataset.AudioDataset.sample_file index:{index}")
        if self.disable_sampling:
            #print(f"$$$$$ sampling disabled, index:{index}")
            return self.meta[index]

        if self.permutation_on_files:
            # print("$$$$$ self.permutation_on_files")
        ...
        if not self.sample_on_weight and not self.sample_on_duration:
            #print("$$$$$ not self.sample_on_weight and not self.sample_on_duration")
            file_index = int(torch.randint(len(self.sampling_probabilities), (1,), generator=rng).item())
        else:
            #print("$$$$$ else")
            file_index = int(torch.multinomial(self.sampling_probabilities, 1, generator=rng).item())

        #print(f"$$$$$ index:{index}; file_index:{file_index}")
        return self.meta[file_index]
    ...
    def __getitem__(self, index: int) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, SegmentInfo]]:
        #print(f"$$$$$ AudioDataset __getitem__")
        if self.segment_duration is None:
            #print("$$$$$ self.segment_duration IS None")
            ...
            #print(f"$$$$$ self.segment_duration IS NOT None: {self.segment_duration}")
            rng = torch.Generator()
            ...
            if self.shuffle:
                ...
                if self.current_epoch is None:
                    #print('$$$$$ shuffle w/o epoch')
                    rng.manual_seed(index + self.num_samples * random.randint(0, 2**24))
                else:
                    #print('$$$$$ shuffle w/ epoch')
            ...
            for retry in range(self.max_read_retry):
                ...
                #print(f"$$$$$ GET file_meta: {file_meta}")
    ...
    def from_meta(cls, root: tp.Union[str, Path], split, **kwargs):
        ...
        """
        #print("data.audio_dataset.AudioDataset.from_meta")
        #print(f"\nfrom_meta kwargs:\n{kwargs}\n")
        root = Path(root)
        if root.is_dir():
