...
class LpCLAPTextConsistencyMetric(TextConsistencyMetric):
    """Text consistency metric relying on Contrastive Language-Audio Pretraining (CLAP) with music description based 
    generate by LP Music Caps.
    ...
    similarity between audio-text pairs. We compute the CLAP embeddings from the text descriptions generated
    for the original music generated by LP Music Caps as well as the generated audio based on the video associated
    with such original soundtrac, and define the MCC metric as the average cosine similarity between these embeddings.

    Model implementation & pre-trained checkpoints: https://github.com/LAION-AI/CLAP https://github.com/FelipeMarra/lp-music-caps https://github.com/seungheondoh/lp-music-caps
    """
    ...
    def update(self, audio: torch.Tensor, videos_paths:tp.List[str], sizes: torch.Tensor, sample_rates: torch.Tensor) -> None:
        """Compute cosine similarity between audio and text pairs and accumulate scores over the dataset."""
        assert audio.size(0) == len(videos_paths), "Number of audio and text samples should match"
        assert torch.all(sample_rates == sample_rates[0].item()), "All items in batch should have the same sample rate"


        music_desc_paths = [v.split('/videos')[0] + '/music_caps/soundtrack_' + v.split('/soundtrack_')[-1].split('/')[0] + '.json' for v in videos_paths]         
        desc_embeddings = []
        for music_desc_path in music_desc_paths:
            with open(music_desc_path, 'r') as f:
                desc_dict = json.load(f)

                descs = []
                for desc in desc_dict.values():
                    descs.append(desc)

            desc_embedding:torch.Tensor = self.model.get_text_embedding(descs, tokenizer=self._tokenizer, use_tensor=True)
            desc_embeddings.append(desc_embedding.mean(0))

        desc_embedding = torch.stack(desc_embeddings)
        ...
        cosine_sim = torch.nn.functional.cosine_similarity(audio_embeddings, desc_embedding, dim=1, eps=1e-8)
